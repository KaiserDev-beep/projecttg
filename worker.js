/**
 * CoinFlip Worker (FULL: setup + webhook + api)
 * Routes:
 *  - GET  /setup?secret=...   -> setWebhook + setChatMenuButton
 *  - POST /webhook            -> Telegram updates (/start)
 *  - POST /api                -> MiniApp API (bet/balance/feed)
 *
 * Required:
 *  - KV binding name: KV
 *  - Secret: BOT_TOKEN_SECRET  (bot token)
 *  - Vars: WEBAPP_URL, SETUP_SECRET
 * Optional:
 *  - Var: WEBHOOK_SECRET (only A-Za-z0-9_-)
 */
...
function text(str, status = 200) {
  return new Response(str, { status, headers: { "content-type": "text/plain; charset=utf-8", ...corsHeaders() } });
}

function safeJsonParse(s, fallback) { try { return JSON.parse(s); } catch { return fallback; } }
function rand(min, max) { return Math.floor(Math.random() * (max - min + 1)) + min; }
function coinFlip() { return Math.random() < 0.5 ? "орел" : "решка"; }
...
// -------- Auth (test mode) --------
// Берём user из body.user (это присылает WebApp), чтобы не упираться в initData.
function getUserFromBody(body) {
  const user = body?.user || null;
  const userId = String(user?.id || body?.userId || "guest");
  const username = user?.username ? `@${user.username}` : (user?.first_name || body?.username || "Player");
  return { userId, username };
}

// -------- handlers --------
async function handleSetup(request, env) {
  const BOT_TOKEN = env.BOT_TOKEN_SECRET;
  const { WEBAPP_URL, SETUP_SECRET, WEBHOOK_SECRET } = env;

  if (!BOT_TOKEN || !WEBAPP_URL || !SETUP_SECRET) {
    return json({ ok: false, error: "Missing env vars: BOT_TOKEN_SECRET / WEBAPP_URL / SETUP_SECRET" }, 500);
  }
  ...
  if (isValidWebhookSecret(WEBHOOK_SECRET)) webhookPayload.secret_token = WEBHOOK_SECRET;
...
async function handleWebhook(request, env) {
  const BOT_TOKEN = env.BOT_TOKEN_SECRET;
  const { WEBAPP_URL, WEBHOOK_SECRET } = env;
  ...
  if (msg?.text && msg?.chat?.id) {
    ...
    if (t.startsWith("/start")) {
      await tgCall("sendMessage", BOT_TOKEN, {
        chat_id: chatId,
        text: "🎮 Готово! Нажми «Открыть игру» или Menu → Играть",
        reply_markup: {
...
async function handleApi(request, env) {
  ...
  if (!BOT_TOKEN) return json({ ok: false, error: "BOT_TOKEN_SECRET missing" }, 500);
  if (request.method !== "POST") return text("Only POST", 405);

  try {
    const body = await request.json().catch(() => null);
    if (!body?.action) return json({ ok: false, error: "Bad request" }, 400);
    ...
    const { userId, username } = getUserFromBody(body);
    ...
    if (body.action === "bet") {
      ...
      const participants = [{ id: userId, name: username, isNpc: false, side, amount }];
      ...
      for (let i = 0; i < npcCount; i++) {
        ...
        participants.push({ id: npc.id, name: npc.name, isNpc: true, side: npcSide, amount: betAmt });
      }

      // списываем ставки
      for (const p of participants) await addBalance(KV, p.id, -p.amount);
      ...
      if (winnersPool > 0) {
        for (const w of winners) {
          ...
          payouts.push({ id: w.id, name: w.name, isNpc: w.isNpc, amount: w.amount, payout, profit: payout - w.amount });
        }
      }
      ...
      const roundEvent = {
        ...
        participants: participants.map(p => ({
          name: p.name, isNpc: p.isNpc, side: p.side, amount: p.amount, win: p.side === result,
        })),
        payouts,
      };
    ...
    return json({ ok: false, error: "API crashed", details: String(e?.stack || e?.message || e) }, 500);
  }
}
